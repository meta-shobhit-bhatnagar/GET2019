		...
		do {
			...
			try {
				...
				case "mkdir":
					...
					int index1;
					for (index1 = 0; index1 < currentDirectory.subDirectory
							.size(); index1++) {
						if (currentDirectory.subDirectory.get(index1).directoryName
								.equals(directory)) {
				...
				case "find":
					...
					if (cmd.findDirectory(currentDirectory, directory) != null)
						System.out.println(cmd.findDirectory(currentDirectory,
								directory));
					else {
				...
				case "ls":
					for (int i = 0; i < currentDirectory.subDirectory.size(); i++) {
						System.out
								.println(currentDirectory.subDirectory.get(i).directoryName);
					}
				...
				System.out
						.println("Warning : Please enter correct type !! Try Again");
				System.out
						.println("-------------------------------------------------------------------------------");
				inputObj.nextLine();
			}
