	...
	Directory changeDirectory(String directoryAddress) {
		...
		while (index != splitAddress.length) {
			...
			for (int i = 0; i < MainDriver.currentDirectory.subDirectory.size(); i++) {
				if (MainDriver.currentDirectory.subDirectory.get(i).directoryName
						.equals(splitAddress[index])) {
					MainDriver.currentDirectory = MainDriver.currentDirectory.subDirectory
							.get(i);
					check++;
				}
			}
